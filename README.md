...
|---|---|---|---|
| [cama](https://github.com/LoriensLibrary/cama) | Core architecture | Operational research deployment (single-participant); multi-tenant 8-layer generalization shipped; 438 tests in CI; `v0.1.0` research release (package `v1.26.0`) | AI safety reviewers, infrastructure engineers |
| [Telos_kalos](https://github.com/LoriensLibrary/Telos_kalos) | Applied prototype | Live deployed app with live Claude integration; 44 tests in CI; `v0.1.0` research release (package `v1.26.0`) | Employers / health-tech / hiring managers |
...
- **[cama](https://github.com/LoriensLibrary/cama)**: Python MCP server implementing the memory architecture. 34 core tools plus 9 optional subsystems on the single-participant side; eight-layer multi-tenant generalization (dyad / hive_protocol / persona / agent / hive_resources / quad / surface / hive_consult) with 438 tests in CI. Single-participant operational deployment (~53,000 memories on the author's instance).
- **[Telos · for Kalos](https://github.com/LoriensLibrary/Telos_kalos)**: full-stack React 19 + TypeScript app with live Claude API integration and a CAMA Proof Layer demonstrating end-to-end provenance trace. 44 tests across 6 suites in CI.
...
1. **[Telos_kalos](https://github.com/LoriensLibrary/Telos_kalos)**: deployed React 19 + TypeScript + Vite app with a serverless Hono + Drizzle + Neon Claude-API route powering the live AI Inbox (rest of the app reads synthetic seeds; full data-layer persistence is roadmap). CAMA Proof Layer demonstrates end-to-end provenance trace. 44 tests in CI. [Live demo](https://telos-kalos.vercel.app).
2. **[cama](https://github.com/LoriensLibrary/cama)**: Python MCP server + eight-layer multi-tenant architecture. Schema enforces provenance discipline. 438 tests in CI. `v0.1.0` research release (package `v1.26.0`).
